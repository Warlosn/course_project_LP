...
namespace MFST
{
	...
	Mfst::Mfst(Lex::TABLES plex, GRB::Greibach pgrebach)//(результат работы лексического анализатора, грамматика Грейбах)
	{
		...
		lenta = new short[lenta_size = lex.Lex.size];//размер ленты = текущий размер таблицы лексем
		for (int k = 0; k < lenta_size; k++)lenta[k] = TS(lex.Lex.table[k].lexema);//заносит в ленту терминалы
		lenta_position = 0;
	...
	const char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_DIGBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: строка %d,%s", err.id, lex.Lex.table[lpos].sn, err.message);
			rc = buf;
		};
		return rc;
	};
