...
namespace PN
{
	...
	void Conversation(LT::LexTable* lextable, IT::IdTable* idtable, int lextable_pos)
	{
		...
		for (int i = lextable_pos; i < len; i++)
		{
			...
			if (t == LEX_ID || t == LEX_LITERALCYMB || t == LEX_LITERALNUM || t == LEX_LITERALCHAR || t == 'w' || t == 'j' || t == 'm' || t == 's')
			{
			...
			if (t == LEX_LEFTHESIS)
			{
				...
				if (t == '+' || t == '-' || t == '*')
				{
					if (!st.size())
						...
						if (ArifmPriorities(t) > pr)																		//если приоритет добавляемой операции больше операции на вершине стека
		...
		for (int i = lextable_pos, k = 0; i < lextable_pos + lenout; i++, k++)
		{
			...
			f = k; //запись в таблицу польской записи
		}
